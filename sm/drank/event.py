...
class Event:
    ...
    def beertank_turfed(self):
        if self._beertank_turfed is None:
    ...
    def beertank_used(self):
        if self._beertank_used is None:
            self._beertank_used = self.bt_deliv.beertank \
                if self.bt_deliv is not None \
                else (self.btc if self.btc is not None else None)
